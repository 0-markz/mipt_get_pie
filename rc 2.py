...
def main(iterations):
    ...
    for i in range(iterations):
        ...
        path='values_'+str(i)+'.csv'
        #df.to_csv(path,index=False)
        with open(path, 'w') as file:
            file.write('Time Voltagedecimal Voltage' + '\n')
            for i in range(len(values)):
                file.write(str(str(times[i])+ ' ' + str(values[i]) + ' ' + str(voltages[i])) +'\n')
        ...
        pathset = 'settings_'+str(i)+'.csv'
        #dfset.to_csv(pathset, index=False)
        with open(pathset, 'w') as file:
            file.write('Charge Discharge Stepvolt Maxvoltdec Minvoltdec Maxvolt Minvolt T')
            file.write( str(str(charge_time) + ' ' + str(discharge_time) +  ' ' + str(((1/maxdec)*mV)) + ' '+ str(maxval) + ' ' + str(minval) + ' ' + str(((maxval/maxdec)*mV)) +  ' ' + str(((minval/maxdec)*mV)) + ' 0.01' ) + '\n')
            for i in range(len(values)):
                file.write(str(str(i) + ' ' + str(values[i]) + ' ' + str(voltages)) + '\n')
        ...
        for i in voltages:
            iteratins.append(c)
            c+=1
        plt.plot(iteratins, voltages)
        plt.xlabel('Iteration')
        plt.ylabel('Voltage')
        plt.title('Charging and discharging RC circuit')
        plt.text(1.1, 1.1, str(str(charge_time)+ ' charge time' + ' ' + str(discharge_time)+ ' discharge_time'))
        plt.grid(c='#BFEFEF',ls='-',lw=0.5)
        plt.show()
